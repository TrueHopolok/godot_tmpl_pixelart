...
@onready var _audio_settings: AudioSettings = get_tree().get_first_node_in_group("settings_audio")
...
func _ready() -> void:
	...
	if _audio_settings.reset:
		value = AudioServer.get_bus_volume_linear(_bus_idx)
	else:
		value = SettingsCfg.config.get_value(
			_audio_settings.AUDIO_SECTION, bus_name, AudioServer.get_bus_volume_linear(_bus_idx)
		)
	_save_volume()
